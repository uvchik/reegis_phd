...
from reegis import entsoe as ents
from reegis import demand_elec
from reegis import geometries
from reegis import openego


def add_grid_labels(data, plotter, label=None, coord_file=None):
...
def de21_region(custom_coordinates=False, draw_02_line=False):
    ...
    if custom_coordinates is True:
        label_coord = os.path.join(
            cfg.get("paths", "geometry"), cfg.get("geometry", "region_label")
        ).format(map="de21")
    else:
        label_coord = None
...
def de21_grid(rmap="de21"):
    ...
    geo = pd.read_csv(
        os.path.join(
            ...
            cfg.get("geometry", "powerlines").format(type="lines", map=rmap),
        ),
        index_col="name",
    )
...
def add_labels(
    data, plot_obj, column=None, coord_file=None, textcolour="blue"
):
    """
...
def coastdat_plot(
    data, data_col=None, lmin=0, lmax=1, n=5, digits=50, legendlabel=""
):

    polygons = pd.read_csv(
        os.path.join(
            cfg.get("paths", "geometry"), "coastdatgrid_polygons.csv"
        ),
        index_col="gid",
    )
    ...
    coastdatplot.geometries = geoplot.postgis2shapely(
        pd.read_csv(
            os.path.join(
                ...
                "data",
                "geometries",
                "region_polygons_de21_simple.csv",
            )
        ).geom
    )
...
def heatmap_pv_orientation():
    ...
    print(df.max().max())
    plt.title(
        "year: 2003, coastDat-2 region: 1129087, "
...
def plot_module_comparison():
    df = pd.read_csv(
        os.path.join(cfg.get("paths", "analysis"), "module_feedin.csv"),
        index_col=0,
    )["dc_norm"]
    print(df)
    ...
    plt.text(
        ...
        500,
        "SF 160S",
        bbox={"facecolor": "white", "alpha": 0.8, "pad": 5, "linewidth": 0},
    )
    plt.text(
        355,
        500,
...
def plot_module_comparison_ts():
    df = pd.read_csv(
        os.path.join(cfg.get("paths", "analysis"), "module_feedin_ac_ts.csv"),
        index_col="Unnamed: 0",
    )
...
def plot_inverter_comparison():
    df = pd.read_csv(
        os.path.join(
            cfg.get("paths", "analyses"), "sapm_inverters_feedin_full2.csv"
        ),
        index_col=[0],
    )["ac"]
...
def plot_full_load_hours(year):
    df = pd.read_csv(
        os.path.join(cfg.get("paths", "analysis"), "full_load_hours.csv"),
        index_col=[0, 1],
    )
    ...
    plt.savefig(
        os.path.join(
            cfg.get("paths", "plots"),
            "full_load_hours_{0}_{1}.svg".format(year, "wind"),
        )
    )
    ...
    for col in columns:
        ...
        coastdat_plot(
            df.loc[year, :][col], lmax=1200, lmin=900, n=7, legendlabel=title
        )
        plt.savefig(
            os.path.join(
                cfg.get("paths", "plots"),
                "full_load_hours_{0}_{1}.svg".format(year, col),
            )
        )
    ...
    columns = [
        "wind",
        "LG290G3_ABB_tltopt_az180_alb02",
    ...
    n = 0
    for col in columns:
        lgnd.get_texts()[n].set_text(
            "{0}, min: {1}, max: {2}".format(
                col,
                int(round(minimum[n] / 10) * 10),
    ...
    for col in columns:
        ...
        ax = fig.add_subplot(1, 1, 1)
        one_region_sort = (
            df.swaplevel(0, 1, axis=0).loc[reg][col].sort_values()
        )
        ...
        plt.savefig(
            os.path.join(
                cfg.get("paths", "plots"),
                "full_load_hours_{0}_region_{1}.svg".format(col, reg),
            )
        )
...
def plot_orientation_by_region():
    df = pd.read_csv(
        os.path.join(
            cfg.get("paths", "analysis"), "optimal_orientation_multi_BP.csv"
        ),
        index_col=[0, 1, 2],
    )
    # print(df)
...
def entsoe_year_plots():
    locale.setlocale(locale.LC_ALL, "de_DE.utf8")
    logger.define_logging()
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    entsoe = ents.split_timeseries_file(version="2019-06-05").load
    for y in range(2006, 2015):
        ...
        de_load_profile = entsoe.loc[start:end].DE_load_
        ax = de_load_profile.resample("M").mean().plot(ax=ax)
    plt.show()


def entsoe_scaled_year_plots():
    locale.setlocale(locale.LC_ALL, "de_DE.utf8")
    logger.define_logging()
    ...
    for y in range(2006, 2015):
        print(y)
        entsoe = demand_elec.get_entsoe_profile_by_region(
            federal_states,
            y,
            "openego_entsoe",
...
def compare_entsoe_slp_annual_profile():
    locale.setlocale(locale.LC_ALL, "de_DE.utf8")
    logger.define_logging()
    my_year = 2014
    federal_states = geometries.get_federal_states_polygon()
...
def compare_entsoe_slp_germany():
    locale.setlocale(locale.LC_ALL, "de_DE.utf8")
    logger.define_logging()
    my_year = 2014
...
def fig_demand_share():
    ...
    open_ego_fs = (
        openego.get_ego_demand_by_region(federal_states, name)
        .groupby(name)
        .sum()
    )
    mydf = (
        open_ego_fs.div(open_ego_fs.sum())
        .mul(100)
        .rename({"consumption": "openEgo"}, axis=1)
    )
    # mydf["renpass"] = demand.renpass_demand_share() * 100
    mydf.sort_values(by="openEgo", inplace=True)
    mydf.plot.bar(stacked=True)
...
if __name__ == "__main__":
    ...
    # entsoe_year_plots()
    # entsoe_scaled_year_plots()
    # compare_entsoe_slp_annual_profile()
    # exit(0)
    # compare_entsoe_slp_germany()
    demand_share_of_sector_and_region()
